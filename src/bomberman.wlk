...
object bomberman {
	...
	method puedoPasar(_direccion) {
		return (self.siguientePosicionEsVacia(_direccion) or self.celdaEsPuertaGanadora(_direccion) or self.celdaEsPowerUp(_direccion) or self.celdaEsVida(_direccion)) and self.validarEjeX(_direccion) and self.validarEjeY(_direccion)
	}
	...
	method comprobarVidaExtra(mensaje) {
		if (self.vidaExtraBomberman().isEmpty()) {
			self.terminarJuego(mensaje)
		} else {
			self.removerVidaExtra()
		}
	}
	...
	method comprobarPowerUps() {
		if (self.powerUps().isEmpty()) {
		} else {
			self.removerSuperBomba()
		}
	}
